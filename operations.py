...
class AuthenticationUser:
	...
	def Query_Permissions(self):
		...
		print(self.request.session['pk_employee'])
		print(values)
	...
	def Login(self):
		...
		try:
			...
			self.request.session['unit'] = values['unit']
			self.request.session['bale'] = values['bale']
			self.request.session['quantity'] = values['quantity']
			self.request.session['img_user'] = values['img_user']			
...
class Inventory:
	...
	def Validated_Quantity(self):
		payload = json.dumps({
		  "pk_employee": self.request.session['pk_employee'],
		  "code": self.request.GET['code'],
		  "type_price": self.request.GET['type_price'],
		  "quantity": self.request.GET['quantity']
		})
		response = requests.request("GET", env.VALIDATED_QUANTITY, headers=self.headers, data=payload)
		return json.dumps(json.loads(response.text))
	...
	def Get_Category(self):
		response = requests.request("GET", env.GET_CATEGORY, headers= self.headers, data=json.dumps({'pk_branch':self.request.session['pk_branch']}))
		return json.loads(response.text)
	...
	def Create_Category(self):
		...
		response = requests.request("POST", env.CREATE_CATEGORY, headers= self.headers, data=payload)
	...
	def Create_Subcategory(self):
		payload = json.dumps({
		  "name": self.request.GET['name'],
		  "pk_category": self.request.GET['pk_category']
		})
		print(payload)
		response = requests.request("POST", env.CREATE_SUBCATEGORY, headers= self.headers, data=payload)
		_response = json.loads(response.text)
		print(_response)
		return json.dumps(_response)
...
class Setting:
	...
	def Get_All_Unit_Measures(self):
		response = requests.request("GET", env.GET_ALL_UNIT_MEASURES, headers= self.headers, data={})
		return json.loads(response.text)
