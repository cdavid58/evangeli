...
from operations import Customer, Setting, Wallet
import env, json
...
def Profile_Customer(request, pk):
	wallet = Wallet(request).Get_All_History_Pass_Customer(pk)
	total = 0
	balance = 0
	for i in wallet:
		total += i['fields']['invoice']['total']
		balance += i['fields']['balance']
	return render(request,'client/profile_customer.html',{'pass_invoice': wallet, 'balance':Thousands_Separator(balance)})