...
var totalSalesEcommerce = function totalSalesEcommerce(salesData, _totals_, _date_invoi) {
  ...
  function getFormatter(params) {
    return params.map(function (_ref16, index) {
      ...
      return "<span class= \"fas fa-circle\" style=\"color: ".concat(borderColor, "\"></span>\n    <span class='text-600'>").concat(index === 0 ? 'Total Vendido' : 'Previous Year', ": ").concat(value, "</span>");
    }).join('<br/>');
  }

  if ($echartsLineTotalSalesEcomm) {
    // Get options from data attribute
    console.log(_totals_+' TOTAL')
    console.log(_date_invoi+' TOTAL')
    ...
    var getDefaultOptions = function getDefaultOptions() {
      return {
        ...
        legend: {
          data: ['Total Vendido', 'previousYear'],
          show: false
        },
        xAxis: {
          type: 'category',
